...
function fetchAllData(routes, params) {
  ...
  }var resolveAllOnObject = Promise.props || Promise.all || function () {};
  return resolveAllOnObject(promises);
}

function renderToDocument(Handler, props, context) {
  return React.render(React.createElement(Handler, props), document.getElementById('app'));
}
...
module.exports = {
  ...
  sync: function sync(routes, opts, cb) {
    ...
    Router.run(routes, loc, function (Handler, state) {
      fetchAllData(state.routes, state.params).then(function (data) {
        var out = render(Handler, { data: data }, opts.context);
  ...
  async: function async(routes, opts, cb) {
    ...
    if (process.env.PLATFORM === 'ios') loc = null;

    Router.run(routes, loc, function (Handler, state) {
      render(Handler, { state: state }, opts.context);
      fetchAllData(state.routes, state.params).then(function (data) {
